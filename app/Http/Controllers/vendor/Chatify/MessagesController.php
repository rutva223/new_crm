...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if(!empty($fetch->avatar))
        {
            ...
            $avatar = \App\Models\Utility::get_file( '/'.$fetch->avatar);
        }
        else
        {
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('name', 'LIKE', "%{$input}%")->where('created_by', '=', \Auth::user()->creatorId());
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        for ($i = 0; $i < count($shared); $i++) {
            $sharedPhotos .= view('Chatify::layouts.listItem', [
                'get' => 'sharedPhoto',
                'image' => \App\Models\Utility::get_file('public/attachments/' . $shared[$i]),
            ])->render();
        }
        // send the response
        return Response::json([
