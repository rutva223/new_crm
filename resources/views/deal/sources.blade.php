{{ Form::model($deal, array('route' => array('deal.sources.update', $deal->id), 'method' => 'post')) }}
    ...
    <div class="row gutters-xs">
        @foreach ($sources as $source)
            <div class="col-12 custom-control custom-checkbox mt-2 mb-2">
                ...
                {{ Form::label('sources_'.$source->id, ucfirst($source->name),['class'=>'custom-control-label ml-4 ms-2 bg-'.$source->color]) }}
            </div>
        @endforeach
    </div>
</div>
